...
var TextContent = /** @class */ (function () {
    ...
    TextContent.prototype.match = function (term) {
        if (this.isBuffer(term) && this.isBuffer(this.getContent())) {
            ...
            var lines = [];
            var line = [];
            for (var i = 0; i < this.getContent().length; i += 1) {
                var chr = this.getContent()[i];
                if (chr === Buffer.from('\n')[0]) {
                    lines.push(line);
                    line = [];
                }
    ...
    var opts = {
        transform: function (raw, encoding, callback) {
            ...
            if (!!args.length && text.search(Buffer.from(args[0]))) {
                ...
                var matches_1 = text.match(Buffer.from(args[0]));
                var lines_1 = [];
                Object.keys(matches_1).forEach(function (k) {
                    lines_1.push(k + ":" + matches_1[k].toString().replace((new RegExp(args[0], 'g')), '\x1b[100m' + args[0] + '\x1b[49m'));
                });
                this.push(Buffer.from(lines_1.join('\n') + "\n"));
            }
            callback();
        }
    };
